...
function config.parse(arg)
    ...
    -- data loader
    cmd:option('-train_file', './data_list/sync_train_list.txt', 'train file, compulsory');
    cmd:option('-test_file',  './data_list/sync_test_list_small.txt', 'test file, compulsory');
    cmd:option('-pretrain_file', '../pre_train_model/sync.t7', 'path to pretrained model');
    cmd:option('-data_root', '********', 'path to suncg data');
	
    -- training and testing
    cmd:option('-gpuid', 1, 'gpu id')
    cmd:option('-optim_state', {rho=0.95, eps=1e-6, learningRate=1e-3, learningRateMin=1e-7, momentum=0.9}, 'optim state')
    cmd:option('-lr_decay', 1000000, 'iterations between lr decreses')
