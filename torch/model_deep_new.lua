...
local function create_addon(c1, c2)
    local conv = nn.Sequential()
    conv:add(nn.SpatialFullConvolution(c1, c2, 3, 3, 1, 1, 1, 1))
    -- conv:add(cudnn.SpatialBatchNormalization(c2, nil, nil, nil))
    conv:add(cudnn.ReLU(true))
...
local function create_model(config)
    ...
    local deconv1 = create_addon(64+64, 3)
    -- local deconv1 = create_deconv_2(64+64, 3)
    -- deconv1:remove(6)
    -- deconv1:remove(5)
    -- local addon = create_addon(3,output_channel)
    ...
    -- local trueoutput = addon(output)


    -- local model = nn.gModule({input}, {trueoutput})
