...
config.result_path = config.result_path .. "_scannet_test_bound/"
...
-- dataset
local train_data = {}
local test_data  = loadScanNetRender(config.test_file, config.root_path)
...
while batch_iterator.epoch==0 and test_count<#batch_iterator.test.data do
    local batch = batch_iterator:nextBatchScanNet('test', config)
